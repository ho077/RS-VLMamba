...
# 新版（正确的导入，兼容当前环境）
# from mmengine.runner import get_root_logger
import logging
# mmengine 仅为日志/权重加载提供便利，未被安装时不影响骨干网络构建
try:
...
def get_root_logger(log_file=None, log_level=logging.INFO):
    """
    手动实现get_root_logger，兼容mmengine 0.10.x系列版本（补充scope必填参数）
    Args:
        log_file (str, optional): 日志文件保存路径. Defaults to None.
        log_level (int, optional): 日志级别. Defaults to logging.INFO.
    Returns:
        logging.Logger: 全局根日志器
    """
    # 初始化默认作用域（补充必填的scope参数，兼容0.10.x版本）
    if init_default_scope is not None:
        init_default_scope(scope="mmengine")
    
    # 获取mmengine全局根日志器
    logger = logging.getLogger('mmengine')
    
    # 避免重复添加日志处理器（多次调用时仅初始化一次）
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            if load_checkpoint is not None:
                ...
                # 未安装 mmengine 时回退到 torch.load，便于仅做前向验证
                state = torch.load(pretrained, map_location='cpu')
                state = state.get('state_dict', state)
                missing, unexpected = self.load_state_dict(state, strict=False)
                logger.info('load %s with missing=%s unexpected=%s', pretrained, missing, unexpected)
        elif pretrained is None:
            self.apply(_init_weights)
        else:
            raise TypeError('pretrained must be a str or None')

    def forward(self, x, l, l_mask,extended_attention_mask):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww,l0= layer(x, Wh, Ww, l0, l_mask,extended_attention_mask,i) 
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        vglva_dims = [128 * 2 ** i for i in range(len(vglva_idx))]
    ...
    def forward(self, x, H, W, l, l_mask,extended_attention_mask,index):
        """ Forward function.
        ...
        # VGLVA fusion
        l_residual = self.vglva[index](l_x, l, extended_attention_mask)
        #print('l_residual.shape=')
        #print(l_residual.shape)
        temp=self.res_gates2[index](l_residual) * l_residual
        #print('temp.shape=')
        #print(temp.shape)
        ...
        if self.downsample is not None:
            ...
            Wh, Ww = (H + 1) // 2, (W + 1) // 2
            return x_residual, H, W, x_down, Wh, Ww,l
        else:
            return x_residual, H, W, x, H, W,l



class LGVLA(nn.Module):
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        super(LGVLA, self).__init__()
...
class VGLVA(nn.Module):
    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=0, dropout=0.0):
        super(VGLVA, self).__init__()
        # input x shape: (B, H*W, dim)
        #self.vis_project = nn.Sequential(nn.Conv1d(dim, dim, 1, 1),  # the init function sets bias to 0 if bias is True
        #                                 nn.GELU(),
        #                                 nn.Dropout(dropout)
        ...
        self.lang_image_att = SpatialLanguageImageAttention(v_in_channels,  # v_in
                                                            l_in_channels,  # l_in
                                                            key_channels,  # key
                                                            value_channels,  # value
                                                            out_channels=value_channels,  # out
...
#         print(l.shape)
        ...
        # print(l_mask.shape)
        lang = self.lang_image_att(x, l, l_mask)  # (B, H*W, dim)
